...
public class StrikeOutAnnotationHandler extends HighLightAnnotationHandler {
    ...
    protected void checkAndApplyPreferences() {
        Color color = null;
        if (preferences.getInt(ViewerPropertiesManager.PROPERTY_ANNOTATION_STRIKE_OUT_COLOR, -1) != -1) {
            int rgb = preferences.getInt(ViewerPropertiesManager.PROPERTY_ANNOTATION_STRIKE_OUT_COLOR, 0);
            color = new Color(rgb);
        }
